...
"""
Edge detection RCF model taken from
...
class RCF(nn.Module):
    weight_deconv2: torch.Tensor
    weight_deconv3: torch.Tensor
    weight_deconv4: torch.Tensor
    weight_deconv5: torch.Tensor
    ...
    def _make_bilinear_weights(self, size: int, num_channels: int):
        factor = (size + 1) // 2
        if size % 2 == 1:
            center = factor - 1
        else:
    ...
    def _crop(self, data: torch.Tensor, img_h: int, img_w: int, crop_h: int, crop_w: int) -> torch.Tensor:
        _, _, h, w = data.size()
        assert (img_h <= h and img_w <= w)
        data = data[:, :, crop_h:crop_h + img_h, crop_w:crop_w + img_w]
        return data

    def forward(self, x: torch.Tensor, apply_sigmoid: bool = True) -> List[torch.Tensor]:
